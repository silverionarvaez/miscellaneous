...
function validateDraft(rule) {
    ...
    else if (state.rules.some((r, i) => i !== state.draftIndex && r.ruleID === rule.ruleID.trim())) {
        errors.push(`Ya existe una regla con ruleID «${rule.ruleID.trim()}».`);
    }
    rule.conditions.forEach((c, i) => {
        if (c.provider === "builtin.xml") {
            if (!c.xpath.trim())
                errors.push(`Condición ${i + 1}: falta la expresión XPath.`);
        }
...
function condToLines(c) {
    ...
    switch (c.provider) {
        ...
        case "builtin.filecontent":
            ...
            lines.push(`  pattern: ${yamlScalar(c.pattern)}`);
            if (c.filePattern.trim())
                lines.push(`  filePattern: ${yamlScalar(c.filePattern)}`);
            break;
        case "builtin.file":
...
function buildRulesYaml() {
    const out = [];
    for (const rule of state.rules) {
        out.push(`- ruleID: ${yamlScalar(rule.ruleID)}`);
        if (rule.description.trim())
            out.push(`  description: ${yamlScalar(rule.description)}`);
        ...
        if (rule.message.trim())
            out.push(`  message: ${yamlScalar(rule.message)}`);
    ...
    switch (c.provider) {
        ...
        <label class="cf">
          ...
          <select data-cf data-ci="${i}" data-field="location">
            ${JAVA_LOCATIONS.map((loc) => `<option value="${loc}" ${c.location === loc ? "selected" : ""}>${loc || "(cualquiera)"}</option>`).join("")}
          </select>
        </label>`;
        case "builtin.filecontent":
            return `
...
function renderDraftForm(rule) {
    const providerHelp = (p) => COND_PROVIDERS.find((cp) => cp.value === p)?.help ?? "";
    return `
    <div class="builder-form">
      ...
      <div class="form-grid">
        ...
        <label class="field">
          ...
          <select id="r-effort">
            ${EFFORT_LEVELS.map((e) => `<option value="${e}" ${rule.effort === e ? "selected" : ""}>${e}</option>`).join("")}
          </select>
        </label>
        <label class="field">
          ...
          <div class="cond-row">
            <div class="cond-head">
              <select data-cprov data-ci="${i}" title="Tipo de condición">
                ${COND_PROVIDERS.map((cp) => `<option value="${cp.value}" ${c.provider === cp.value ? "selected" : ""}>${cp.label}</option>`).join("")}
              </select>
              ${rule.conditions.length > 1
...
function renderBuilderPane() {
    const list = state.rules.length
        ...
        ${state.rules
          ...
          <li class="rule-card">
            <div class="rule-card-main">
              ...
              <p class="rule-when">
                ${r.conditions.map((c) => `<code>${esc(condSummary(c))}</code>`).join(` <em>${r.logic === "or" ? "OR" : "AND"}</em> `)}
              </p>
            </div>
        ...
        : "";
    const empty = state.rules.length === 0 && !state.draft
        ...
        : "";
    const toolbar = state.rules.length > 0 && !state.draft
        ...
        : "";
    const yamlPreview = state.rules.length > 0
...
function renderStep3() {
    ...
    const rulesSummary = state.mode === "create"
    ...
    if (state.created) {
        const uploadLine = state.mode === "create"
...
function startDraft(index) {
    ...
    state.draft = index >= 0 ? JSON.parse(JSON.stringify(state.rules[index])) : newRule();
    render();
}
function bindBuilder() {
    document.querySelector("#btn-first-rule")?.addEventListener("click", () => startDraft(-1));
    document.querySelector("#btn-new-rule")?.addEventListener("click", () => startDraft(-1));
    ...
    document.querySelector("#r-cat")?.addEventListener("change", (e) => {
        draft.category = e.target.value;
    });
    document.querySelector("#r-effort")?.addEventListener("change", (e) => {
        draft.effort = Number(e.target.value);
    });
    document.querySelector("#r-logic")?.addEventListener("change", (e) => {
        draft.logic = e.target.value;
    });
    // Condiciones
    document.querySelectorAll("[data-cf]").forEach((input) => {
        input.addEventListener("input", () => {
            ...
            if (field !== "provider") {
                draft.conditions[ci][field] = input.value;
            }
        });
    });
    document.querySelectorAll("[data-cprov]").forEach((sel) => {
        sel.addEventListener("change", () => {
            draft.conditions[Number(sel.dataset.ci)].provider = sel.value;
            render();
        });
    });
    document.querySelectorAll("[data-rm-cond]").forEach((btn) => {
        btn.addEventListener("click", () => {
    ...
    document.querySelector("#btn-add-cond")?.addEventListener("click", () => {
        draft.conditions.push(newCondition());
        if (draft.logic === "single")
            draft.logic = "and";
        render();
    });
    $("#btn-save-rule").addEventListener("click", () => {
        ...
        if (state.draftIndex >= 0)
            state.rules[state.draftIndex] = draft;
        else
...
function downloadPayload() {
    downloadBlob(JSON.stringify(buildPayload(), null, 2), "application/json", `target-${slugify(state.name) || "konveyor"}.json`);
}
...
function bindStep3() {
    if (state.created) {
        ...
        document
            .querySelector("#btn-download-yaml-2")
    ...
    document
        .querySelector("#btn-download-yaml")
        ?.addEventListener("click", downloadRulesYaml);
